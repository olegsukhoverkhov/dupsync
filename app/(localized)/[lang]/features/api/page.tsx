...
const TRANSLATIONS = {
  ...
  zh: {
    title: "视频配音API — 集成到您的产品 | DubSync",
    description:
      "使用DubSync REST API将视频配音集成到您的产品中。",
    badge: "API",
    h1: "视频配音API",
    h1Highlight: "面向开发者和团队",
    subtitle:
      "将AI视频配音直接集成到您的产品中。一次API调用即可完成转录、翻译、语音克隆和30多种语言的口型同步。",
    ctaApi: "获取API访问权限",
    ctaDocs: "阅读文档",
    codeTitle: "一次调用即可",
    codeHighlight: "配音视频",
    codeFooter: "完整API参考和代码示例见",
    codeFooterLink: "文档",
    capabilitiesTitle: "核心功能",
    techSpecsTitle: "技术规格",
    pricingTitle: "API定价",
    pricingDesc: "API访问包含在Pro和Enterprise方案中。Pro起步$29/月，含60分钟配音。Enterprise包含自定义批量定价和优先处理。",
    pricingLink: "查看完整定价",
    ctaTitle: "开始使用DubSync API构建",
    ctaSubtitle: "创建账户，生成API密钥，5分钟内提交第一个配音任务。",
    ctaButton: "免费开始",
    ctaDocs2: "API文档",
    faqTitle: "常见问题",
    relatedTitle: "相关功能",
    allFeatures: "所有功能",
    capabilities: [
      { title: "一次调用配音", description: "发送视频URL、目标语言和选项到单个POST请求。DubSync从头到尾处理转录、翻译、语音克隆和口型同步。" },
      { title: "Webhook通知", description: "注册Webhook URL，在任务开始、完成或遇到错误时接收实时更新。" },
      { title: "批量处理", description: "在单个请求中提交多个视频。每个视频并行处理，结果单独交付。" },
      { title: "脚本访问", description: "通过API检索转录、翻译脚本和同步数据。以编程方式编辑脚本。" },
      { title: "默认安全", description: "所有API流量使用TLS 1.3加密。API密钥具有特定权限，可随时轮换。" },
      { title: "使用量跟踪", description: "通过专用端点查询当前使用量、剩余积分和任务历史。" },
    ],
    faqs: [
      { question: "哪些方案包含API访问？", answer: "API在Pro和Enterprise方案中可用。Pro包含每分钟60个请求。Enterprise解锁更高吞吐量和优先处理。" },
      { question: "如何验证API请求？", answer: "在DubSync控制面板的设置 > API密钥中创建API密钥。在每个请求的Authorization头中作为Bearer令牌传递。" },
      { question: "有测试沙盒环境吗？", answer: "有。每个API密钥默认在沙盒模式下工作。沙盒请求免费处理30秒预览。" },
      { question: "配音任务失败会怎样？", answer: "失败的任务返回详细的错误对象，包含机器可读的错误代码和人类可读的消息。" },
    ],
